...
const adminLinks = [
  ...
  { label: '회원', href: '/admin?tab=users', icon: Users },
  { label: '게시글', href: '/admin?tab=posts', icon: FileText },
  { label: '댓글', href: '/admin?tab=comments', icon: MessageSquare },
  { label: '공지', href: '/admin?tab=notices', icon: Bell },
  { label: '문의', href: '/admin?tab=inquiries', icon: Inbox },
  { label: '신고', href: '/admin?tab=reports', icon: Flag },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 min-h-16 flex items-center justify-between gap-4">
        <div className="flex items-center gap-5 min-w-0">
          <Link href="/admin" className="flex items-center gap-3 shrink-0">
            <span className="w-9 h-9 rounded-lg bg-primary text-white flex items-center justify-center"><ShieldCheck size={20} /></span>
          ...
          <div className="hidden lg:flex items-center gap-1">
            {adminLinks.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} className={`flex items-center gap-1.5 px-2.5 py-2 rounded-lg text-sm font-bold ${active ? 'bg-primary/15 text-primary' : 'text-text-secondary hover:bg-surface hover:text-text-primary'}`}>
